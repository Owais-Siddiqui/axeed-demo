  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        className="border border-slate-600 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 w-full flex items-center justify-between bg-slate-800"
      >
      ...
      {open && (
        <div className="absolute z-10 mt-1 w-full bg-slate-800 border border-slate-700 rounded-lg shadow-lg py-1">
          {SKILL_OPTIONS.map(skill => (
            <label
              key={skill}
              className="flex items-center gap-2.5 px-3 py-2 hover:bg-slate-700 cursor-pointer text-sm text-slate-200"
            >
              <input
                type="checkbox"
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="mb-4 relative">
        ...
        <input
          ...
          className="border border-slate-600 rounded-lg pl-9 pr-3 py-2 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 w-full max-w-sm bg-slate-800"
        />
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-800 border-b border-slate-700">
              <tr>
                {["Name", "Phone", "Skills", "Status", "Open Tickets", ""].map(h => (
                  <th key={h} className="px-4 py-3 text-left text-xs font-semibold uppercase text-slate-400 whitespace-nowrap">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-slate-700/50">
              {paged.map(w => (
                ...
                >
                  <td className="px-4 py-3 font-semibold text-white whitespace-nowrap">{w.full_name}</td>
                  <td className="px-4 py-3 text-slate-400 whitespace-nowrap">{w.phone}</td>
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1">
                      {w.skills.map(skill => (
                        <span key={skill} className="text-xs bg-slate-700 text-slate-300 px-2 py-0.5 rounded-full capitalize">
                          {skill}
                        </span>
                      ))}
                    </div>
                  ...
                  <td className="px-4 py-3 text-slate-300 font-mono">{w.open_tickets}</td>
                  <td className="px-4 py-3">
                    <button
      ...
      {showModal && (
        ...
        >
          <div className="bg-slate-800 border border-slate-700/50 rounded-2xl shadow-2xl animate-scaleIn p-6 max-w-md w-full mx-4 mt-16 mb-8" onClick={e => e.stopPropagation()}>
            ...
            {saveError && (
              <p className="text-sm text-rose-400 bg-rose-500/10 border border-rose-500/20 rounded-lg px-3 py-2 mt-4">
                {saveError}
              </p>
            )}
